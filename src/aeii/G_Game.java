...
public final class G_Game extends F_StringManager implements Runnable,
	...
	private A_Unit var_493a;
	private A_Unit var_4942;
	...
	private boolean sub_5d81() {
		return (this.var_4a4a) && (F_StringManager.mainCanvas.var_17b7 == this);
	}
	...
	private void loadSlot(byte[] data) {
		...
		try {
			...
			if ((this.var_47a2 == 6) && (this.var_4e12 > 32)) {
				this.var_49c2 = showMessageBox(
						F_StringManager.getLangString(121 + this.var_47a2),
	...
	private void sub_70c0(F_StringManager paramClass_f_0145) {
		int i;
		...
		if (k == 0) {
			...
			this.someMenu.sub_20ad((String[]) localObject2, this.someCanWidthDiv2,
			...
			int j = (this.someCanHeight - this.var_4c1a.mapPrevPixelHeight
					- this.someMenu.mapPrevPixelHeight + localClass_e_0134.mapPrevPixelHeight) / 2;
	...
	private E_Menu sub_7384(F_StringManager inSM) {
		...
		menu1.someImage1 = this.menuIconsFrames[6]; // about Icon
		this.someHoverMenu.sub_20ad(this.settingsStrListMaybe, this.canvasWidth / 2,
	...
	private static DataInputStream sub_74ac(int paramInt) {
		if (paramInt >= someOneOf12LangStrings.length) {
			...
			return new DataInputStream(new ByteArrayInputStream(
					C_MainCanvas.getRecordData("download", paramInt)));
		}
		return new DataInputStream(C_MainCanvas.getResourceStream("s" + paramInt
				+ ".aem"));
	}
	...
	private static byte[] sub_751c(int paramInt) {
		if (paramInt >= someOneOf12LangStrings.length) {
			paramInt -= someOneOf12LangStrings.length;
			return C_MainCanvas.getRecordData("download", paramInt);
		}
		try {
			InputStream stream;
			stream = C_MainCanvas.getResourceStream("s" + paramInt); // SKIRMISH MApS
															// @todo
								...
								if (paramClass_e_0134 == this.var_4a82) {
									...
									if (paramClass_e_0134 == this.var_4be2) {
													...
													.sub_1ca8(
															...
															F_StringManager
																	.getStrByIdAndReplaceUWith(
																			54,
																			sub_7651(this.var_4efa[this.someMenu.var_11c2[paramInt]]))
																	+ "\n"
																	+ F_StringManager
																			.getStrByIdAndReplaceUWith(
																					53,
										...
										if (paramByte == 0) {
											...
											E_Menu cl0134 = sub_7384(this.someHoverMenu.stringManager);
	...
	private A_Unit sub_ac11(byte paramByte, int paramInt1, int paramInt2) {
		this.var_486a[this.someUnitTeamId] -= A_Unit.unitsCost[paramByte];
		this.var_4c42 = true;
		return A_Unit.createUnit(paramByte, this.someUnitTeamId, paramInt1,
				paramInt2);
	}
	...
	private void sub_aca4(int paramInt) {
		try {
			...
			if (this.var_45fa == 0) {
				this.var_4b52 = F_StringManager.getLangString(paramInt + 113);
			...
			for (int n = 0; n < this.var_49fa.length; n = (short) (n + 1)) {
				...
				if ((i1 > 0) && ((i2 < 0) || (this.var_487a[i2] == 2))) {
					changeTileToOccupiedByTeam(this.var_49fa[n][0], this.var_49fa[n][1], 0);
				}
			}
			...
			for (int n = 0; n < i1; n = (short) (n + 1)) {
				...
				if (this.var_487a[b2] != 2) {
					A_Unit localClass_a_0260 = A_Unit.createUnit(b1,
							b2, m, j);
					if (b1 == 9) {
			...
			if (this.var_45fa == 0) {
				this.var_49c2 = showMessageBox(
						F_StringManager.getLangString(121 + this.var_47a2),
						F_StringManager.getLangString(129 + this.var_47a2),
						this.someCanHeight, -1);
			} else {
				this.var_49c2 = (this.var_49c2 = showMessageBox(
			...
			if (F_StringManager.mainCanvas.var_17b7 == this) {
				...
				if (this.var_4912) {
							...
							if (this.var_478a == 13) {
								...
								if (this.var_4b32 != null) {
									...
									if (this.var_4ae2.size() > 0) {
										localClass_a_0260 = (A_Unit) this.var_4ae2
												.elementAt(0);
										if (this.var_4af2 == 0) {
												...
												if (this.var_478a == 1) {
													...
													if (this.var_4f92 == 1) {
														...
														if (this.var_4f92 == 2) {
															...
															A_Unit localObject51 = this.teamsUnits[this.someUnitTeamId][((this.unitAtCursorPos.unitId + 1) % this.teamsUnitsCount[this.someUnitTeamId])];
															for (;;) {
																i4++;
					...
					for (int k = this.var_488a.size() - 1; k >= 0; k--) {
						...
						(localClass_g_25174 = (G_Sprite) this.var_488a
		...
		if (this.gameStateMb == 0) { // waving image state
			...
			if ((this.var_478a == 11) && (!this.var_4912)) {
				...
				if (this.var_47f2) {
				 ...
				 muIndex = 0;
				mapUnitCount = this.var_488a.size();
				while ( muIndex < mapUnitCount) {
					G_Sprite lob = (G_Sprite) this.var_488a.elementAt( muIndex);
					lob.sub_1d20(
									graphics,
									this.mapLeftX,
									this.mapTopY
				...
				if (this.var_4c42) {
					...
					if (this.var_4f92 == 1) {
						...
						if ((!C_MainCanvas.propDeveloperModeOn)
							...
							C_MainCanvas.sub_2007(graphics, ""
									+ this.var_486a[this.someUnitTeamId], i3count, i2, 1,
									6);
						}
					}
					graphics.setClip(0, 0, this.canvasWidth, this.canvasHeight);
	...
	private void sub_122cc() {
		A_Unit[] units = getSomeUnitsOfTypeOfTeam(-1, 0, this.someUnitTeamId);
						...
						if ((countSomeTeamUnitsOfType(0, -1, this.someUnitTeamId) < 2)
							...
							if ((this.var_486a[this.someUnitTeamId] >= 1000)
								...
								for (byte i71 = 1; i71 < 12; b = (byte) (i71 + 1)) {
									if (((countSomeTeamUnitsOfType(i71, -1, this.someUnitTeamId) <= 0) || (A_Unit.unitsCost[i71] >= 600))
											&& (sub_12199(i71, m, n))) {
										arrayOfByte[i5] = i71;
										i5++;
									}
	...
	private void someParseScriptMethod() {
		...
		while ((i >= 0) && (i < scriptArr2.length)) {
			...
			if ((localObject1 = scriptArr2[(i++)])[0]
				...
				showUnitDialog(
						F_StringManager.getLangString(Integer.parseInt(localObject1[1])),
				...
				C_MainCanvas.sub_2af6(Integer.parseInt(localObject1[1]));
			} else {
				Object localObject2;
				if (localObject1[0].equalsIgnoreCase("MoveMapAndCursor")) {
					if (localObject1[1].equalsIgnoreCase("king")) {
						...
						sub_186a6(((A_Unit) localObject2).posX,
								((A_Unit) localObject2).posY);
					} else {
					...
					showSpriteOnMap((G_Sprite) localObject2, unit.pixelX, unit.pixelY, // m, n pos
							Integer.parseInt(localObject1[2]),
							Integer.parseInt(localObject1[3]),
							Integer.parseInt(localObject1[4]),
							Integer.parseInt(localObject1[5]));
			...
			if (this.var_4e3a) {
				...
				if (this.var_45fa == 1) {
					if (this.var_4e12 == 100) {
						...
						this.var_49c2.sub_132e((byte) 0, true);
						this.var_49c2.sub_1350(null);
						F_StringManager.mainCanvas.sub_220e(this.var_49c2);
						this.var_4e12 += 1;
						return;
					}
				...
				} else if ((this.gameStateMb == 1) && (this.var_45fa == 0)
						&& (this.var_4e12 != -1)) {
					if (this.var_4e42 != -1) {
					...
					if (this.someScriptStrings != null) {
						...
						if (this.var_47a2 == 0) {
							...
							case 6:
								...
								C_MainCanvas.sub_2af6(100);
								((A_Unit) localObject2).sub_108f(400);
								showSpriteOnMap(this.redsparkSprite,
							...
							case 8:
								...
								showSpriteOnMap(this.smokeSprite,
										...
										1, 100);
								((A_Unit) localObject1).killUnitMaybe();
								sub_142e5(20);
								this.var_4e12 += 1;
								break;
							case 9:
							...
							case 32:
								this.var_4a4a = true;
								this.var_47f2 = true;
								this.var_49c2 = showMessageBox(F_StringManager
			...
			if (inparamCommand == this.var_506a) {
				...
				try {
					...
					try {
						...
						paramCommand.var_4d6a = new A_Unit[paramCommand.var_49fa.length];
						paramCommand.var_4d72 = new byte[paramCommand.var_49fa.length];
						paramCommand.var_4a6a = new G_Sprite[paramCommand.var_49fa.length];
